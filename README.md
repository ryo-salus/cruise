...
画面表示はすべて日本語。質問文には日本語訳が並ぶ。
**回答案だけが英語**で、それがそのまま相手に送る文章になる。
...
| status | 画面の表示 | 意味 |
|---|---|---|
| `confirmed` | そのまま使えます | 複数ソースで一致 |
| `single_source` | 念のため目を通す | 1ソースのみ |
| `conflict` | 値が食い違い。自分で決める | 両方の値と出典が並んで出る |
| `placeholder` | 今回の値を入れる | 寄港ごと・船社ごとに変わる |
| （該当なし） | 台帳にないので自分で答える | 空欄で出る |
